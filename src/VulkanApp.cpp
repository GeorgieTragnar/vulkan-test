...
void VulkanApp::pickPhysicalDevice()
{
	...
	for (const auto& device : devices) {
		...
		if (suitable && extensionsSupported && swapChainAdequate && supportedFeatures.samplerAnisotropy) 
		{
...
void VulkanApp::createLogicalDevice()
{
	...
	_deviceFeatures.sampleRateShading = VK_TRUE; // enable sample shading feature for the device
...
void VulkanApp::createTextureImage()
{
	...
	_textureMipLevels = static_cast<uint32_t>(std::floor(std::log2(std::max(texWidth, texHeight)))) + 1;
	...
	createImage(texWidth, texHeight, _textureMipLevels, 
	...
	transitionImageLayout(_textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, _textureMipLevels);
	...
	generateMipmaps(_textureImage, VK_FORMAT_R8G8B8A8_SRGB, texWidth, texHeight, _textureMipLevels);
}

void VulkanApp::createTextureImageView()
{
	_textureImageView = createImageView(_textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT, _textureMipLevels);
}
...
void VulkanApp::createTextureSampler()
{
	...
	samplerInfo.maxLod = static_cast<float>(_textureMipLevels);
...
void VulkanApp::createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, 
